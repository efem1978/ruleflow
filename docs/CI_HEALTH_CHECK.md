...
- VS Code 扩展（阶段性策略）：默认仅警告阈值为 ≥80%（非阻断）；可通过设置 `VSCODE_COVERAGE_GATE=1` 启用硬门禁（同阈值）。失败时输出近阈值/最低覆盖文件。
  - 本仓库标准：CI 启用 98% 硬门禁（`VSCODE_COVERAGE_GATE=1` 且 `VSCODE_COVERAGE_THRESHOLD_WARN=98`），失败时导出 near/worst 清单以便补测。
...
- 近阈值窗口（near）调优：
  - 当 Coverage Gate 已通过（weak=0）但 near 列表干扰聚焦时，可按需将 `coverage.near.within` 调整为 0.8–3.0% 的合理区间，仅影响 near 报告与优先级，不改变门槛；
  - 本仓库当前设置为 0.8% 以清空 near 列表，便于将注意力集中在真正弱项（weak）与分组薄弱点上。
- 类型检查失败：
...
通过标准（DoD）
- 所有 Job 成功；Coverage Policy Gate 无 weak；VS Code 覆盖率≥80%（默认仅警告；若启用门禁则需达标）。本仓库执行标准为 98% 硬门禁。
