...
- 面板帮助入口：
  - 提供“打开用户上手 / Open User Guide”与“打开 IDE 支持 / Open IDE Support”按钮
  - 可一键打开 `docs/USER_GUIDE.md` 与 `docs/IDE_SUPPORT.md`
- 新手建议短语：
...
- Dev Agent 自动记忆（可选）：
  - 设置 `DEV_AGENT_MEM_ENABLE=1` 后按最小间隔（`DEV_AGENT_MEM_MIN_SEC`，默认 600s）追加简要摘要到 `.mcp/memory.json`
  - 窗口 `DEV_AGENT_MEM_MAX_TURNS`（默认 20）
  - 摘要仅包含整体进度/弱项计数/当前步骤，不含源码内容
...
- 严格隔离：
  - 默认以 `MCP_STRICT_ISOLATION=1` 启动后端
  - 仅当 `.mcp/assistant.yaml` 设置 `memory.allow_write: true` 时允许写记忆
  - 环境变量无法开启写入
