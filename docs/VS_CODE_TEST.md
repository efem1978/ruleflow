...
1. 打开面板
1. 点击“摄取规则 / Ingest”，输入示例：`README.md, docs/`；
   或用 `RuleFlow: Natural Command` 输入“摄取规则 README.md, docs/”
1. 点击“载入编译规则 / Load Rules” 与 “载入建议 / Load Suggestions”，确认展示内容
1. 点击"加载覆盖率 / Load Coverage"
...
1. CI 操作
   ...
   - 覆盖率策略（阶段性）：
     - 非阻断告警：CI 对 `lcov.info` 执行 80% 检查，低于阈值仅发出警告：
       `sh scripts/check-lcov.sh extensions/vscode/coverage/lcov.info 80`
     - 可选硬门禁（默认关闭）：开启 `VSCODE_COVERAGE_GATE=1` 后使用同阈值作为门禁：
...
1. 可选：环境准备（便于隔离验证）
   ...
   - 面板快速预览：点击"准备环境(预览) / Prepare Env (dry-run)"按钮，弹出计划（不实际执行）

1. 受限环境的 VS Code 测试

   - 建议优先：清空默认参数后重试：
     `export MCP_VSCODE_TEST_ARGS="" && npm --prefix extensions/vscode test`
   - 若仍失败，可改用自定义启动参数（逗号分隔）：
     `export MCP_VSCODE_TEST_ARGS="--disable-extensions"`
