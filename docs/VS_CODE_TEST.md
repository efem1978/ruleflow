...
5) CI 操作
   ...
   - 覆盖率门禁：CI 中已启用 VS Code lcov 覆盖率门禁（当前 80%），测试脚本会生成 `extensions/vscode/coverage/lcov.info` 后以门禁模式校验：
     - 核心命令：`sh scripts/check-lcov.sh extensions/vscode/coverage/lcov.info 80 gate`
