...
- VS Code 面板：点击“插入示例规则”即可在项目根生成
  `.hadolint.yaml` 与 `.semgrep.yml`（可编辑）。
- CI：
  - hadolint：`mcp-rules-assistant ci-set --hadolint`
    （可用 `--hadolint-image/--hadolint-args` 定制）
  - semgrep：`mcp-rules-assistant ci-set --semgrep-config p/ci`
    （或使用仓库根的 `.semgrep.yml`）
- CLI：`mcp-rules-assistant insert-security-samples`
  可快速插入示例 `.semgrep.yml` 与 `.hadolint.yaml`
