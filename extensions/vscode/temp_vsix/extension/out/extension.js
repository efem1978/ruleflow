...
function getWorkspaceLabel() {
    try {
        if (__lockedRoot) {
            try {
                return path.basename(__lockedRoot);
            }
            catch { }
        }
        const ed = vscode.window.activeTextEditor;
...
class McpClient {
    constructor() {
        ...
        this.fakeMode = ((process.env.RULEFLOW_TEST_FAKE || '').trim() === '1');
        this.fakeCalls = [];
        this.hb = null;
        ...
        this.autoPrepared = false;
        this.outBuf = '';
    }
    updateFakeMode() {
        try {
            if ((process.env.RULEFLOW_TEST_FAKE || '').trim() === '1') {
                this.fakeMode = true;
                return;
            }
            const ws = getWorkspaceRoot() || process.cwd();
    ...
    start(context) {
        ...
        this.proc = (0, child_process_1.spawn)(pyBin, ['-m', 'mcp_rules_assistant.cli', 'start'], {
            cwd: ws,
                ...
                try {
                    if (!this.autoPrepared && /No module named .*mcp_rules_assistant/.test(String(chunk || ''))) {
                        ...
                        const run = (cmd, args) => new Promise((resolve) => {
                            try {
                                const p = (0, child_process_1.spawn)(cmd, args, { cwd: ws });
                                p.on('close', () => resolve());
                                p.on('error', () => resolve());
                            }
                            catch {
        ...
        this.proc.on('close', (code) => {
            ...
            this.connected = false;
            try {
                vscode.commands.executeCommand('setContext', 'ruleflow.mcpConnected', false);
            }
            catch { }
        ...
        this.hb = setInterval(async () => {
            try {
                ...
                if (ok !== this.connected) {
                    this.connected = ok;
                    try {
                        vscode.commands.executeCommand('setContext', 'ruleflow.mcpConnected', ok);
                    }
                    catch { }
                }
            }
    ...
    async _fakeRequest(method, params) {
        try {
            // \u8bb0\u5f55\u8c03\u7528\u4f9b\u6d4b\u8bd5\u65ad\u8a00
            try {
                this.fakeCalls.push({ method, name: (params && params.name) || undefined, params, args: (params && (params.arguments ?? params)) });
            }
            catch { }
            const ws = getWorkspaceRoot() || process.cwd();
            const fsApi = vscode.workspace.fs;
            const fileToString = async (u) => {
                try {
                    const b = await fsApi.readFile(u);
            ...
            if (method === 'resources/list') {
                ...
                try {
                    await fsApi.stat(vscode.Uri.file(ws + '/.mcp/dashboard/notree'));
                    noTree = true;
                }
                catch { }
                try {
                    await fsApi.stat(vscode.Uri.file(ws + '/.mcp/dashboard/norules'));
                    noRules = true;
                }
                catch { }
                if (!noRules) {
                ...
                if (!noTree)
                    items.push({ uri: `coverage://project/x/tree`, name: 'Coverage tree' });
                ...
                try {
                    await fsApi.stat(vscode.Uri.file(ws + '/.github/workflows/ci.yml'));
                    items.push({ uri: `ci://project/x/workflow`, name: 'CI workflow (YAML)' });
                }
                catch { }
            ...
            if (method === 'resources/read') {
                ...
                if (uri.endsWith('/summary')) {
                    let bad = false;
                    try {
                        await fsApi.stat(vscode.Uri.file(ws + '/.mcp/dashboard/coverage_bad'));
                        bad = true;
                    }
                    catch { }
                    if (bad) {
            ...
            if (method === 'tools/call') {
                ...
                if (name === 'coverage.near') {
                    let empty = false;
                    try {
                        await fsApi.stat(vscode.Uri.file(ws + '/.mcp/dashboard/near_empty'));
                        empty = true;
                    }
                    catch { }
                    if (empty)
                ...
                if (name === 'ci.generate') {
                    ...
                    try {
                        await fsApi.createDirectory(vscode.Uri.file(ws + '/.github/workflows'));
                    }
                    catch { }
                ...
                if (name === 'ci.validate') {
                    try {
                        await fsApi.stat(vscode.Uri.file(ws + '/.github/workflows/ci.yml'));
                        return { ok: true, checks: { workflow: true } };
                    }
                    catch {
                        return { ok: false, checks: { workflow: false } };
                    }
                }
                ...
                if (name === 'plan.set') {
                    ...
                    try {
                        await fsApi.createDirectory(vscode.Uri.file(ws + '/.mcp'));
                    }
                    catch { }
...
let __testWebviewHandler = null;
let __testPanelHandler = null;
let __panelReadyResolve = null;
let __panelReady = null;
let __panelInFlightResolve = null;
let __panelInFlight = null;
const __ready2Resolvers = new Map();
const __ready2Promises = new Map();
...
function activate(context) {
    ...
    try {
        ...
        const isUnderAnyWs = (p) => { try {
            const n = path.resolve(p);
            return wsFolders.some(w => n.startsWith(path.resolve(w)));
        }
        catch {
            return false;
        } };
        const pathExists = (p) => { try {
            return fs.existsSync(p);
        }
        catch {
            return false;
        } };
        const saved = (context.workspaceState.get('ruleflow.lockRoot') || '').trim();
        if (saved && pathExists(saved) && isUnderAnyWs(saved)) {
            ...
            if (w0) {
                ...
                try {
                    ...
                    if (pr && pathExists(pr) && isUnderAnyWs(pr)) {
                        ...
                        __lockedRoot = null;
                        try {
                            const s = { ...(obj || {}), projectRoot: w0 };
                            fs.mkdirSync(path.dirname(prefs), { recursive: true });
                            fs.writeFileSync(prefs, JSON.stringify(s, null, 2));
                        }
                        catch { }
                    }
                }
            ...
            if (saved && (!pathExists(saved) || !isUnderAnyWs(saved))) {
                try {
                    context.workspaceState.update('ruleflow.lockRoot', '');
                }
                catch { }
            }
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.quickActions', async () => {
        ...
        if (choice === 'Backend Doctor') {
            await vscode.commands.executeCommand('mcpRulesAssistant.backendDoctor');
            return;
        }
        if (choice === 'Load Coverage') {
        ...
        if (choice === 'Open Status') {
            await vscode.commands.executeCommand('mcpRulesAssistant.openStatus');
            return;
        }
        if (choice === 'Ingest Rules (Quick)') {
            await vscode.commands.executeCommand('mcpRulesAssistant.ingestQuick');
            return;
        }
        if (choice === 'Status Update') {
            await vscode.commands.executeCommand('mcpRulesAssistant.statusUpdate');
            return;
        }
        if (choice === 'Generate CI') {
            await client.request('tools/call', { name: 'ci.generate', arguments: {} });
            vscode.window.showInformationMessage('CI \u5df2\u751f\u6210');
            try {
                if (memAllowed()) {
                    await client.request('tools/call', { name: 'memory.append_turn', arguments: { role: 'assistant', content: 'CI generated', meta: { source: 'vscode', action: 'ci.generate' } } });
                }
            }
            catch { }
            return;
        }
        if (choice === 'Validate CI') {
            await client.request('tools/call', { name: 'ci.validate', arguments: {} });
            vscode.window.showInformationMessage('CI \u6821\u9a8c\u5b8c\u6210');
            try {
                if (memAllowed()) {
                    await client.request('tools/call', { name: 'memory.append_turn', arguments: { role: 'assistant', content: 'CI validated', meta: { source: 'vscode', action: 'ci.validate' } } });
                }
            }
            catch { }
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.statusUpdate', async () => {
        try {
            ...
            execFile(pyBin, ['-m', 'mcp_rules_assistant.cli', 'status-update', '--json'], { cwd, env }, async (err, stdout, stderr) => {
                if (err) {
                ...
                try {
                    ...
                    const w = (cov.weak || []).length || 0;
                    const n = (cov.near || []).length || 0;
                    const mm = cov.min_module;
            ...
            vscode.window.showErrorMessage('\u72b6\u6001\u5237\u65b0\u5931\u8d25\uff1a' + String(e));
        }
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.ingestQuick', async () => {
        ...
        try {
            await client.request('tools/call', { name: 'rules.ingest', arguments: { paths: ['README.md', 'docs/'] } });
            vscode.window.showInformationMessage('\u89c4\u5219\u6444\u53d6\u5b8c\u6210');
        }
        catch (e) {
            vscode.window.showErrorMessage('\u89c4\u5219\u6444\u53d6\u5931\u8d25\uff1a' + String(e));
        }
    }));
    // \u6253\u5f00\u72b6\u6001\u6587\u4ef6
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.openStatus', async () => {
        try {
            ...
            vscode.window.showInformationMessage('status.json not found');
        }
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.memoryAppendSelection', async () => {
        ...
        if (!text) {
            text = await vscode.window.showInputBox({ placeHolder: '\u8f93\u5165\u8981\u8ffd\u52a0\u5230\u8bb0\u5fc6\u7684\u6587\u672c' }) || '';
        }
        if (!text) {
            vscode.window.showWarningMessage('\u65e0\u5185\u5bb9\u53ef\u8ffd\u52a0');
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.fsApplyPatch', async () => {
        ...
        if (modeTop.startsWith('single')) {
            ...
            if (ed) {
                ...
                if (p && p.startsWith(ws))
                    defaultPath = p.substring(ws.length + 1).replace(/\\\\/g, '/');
            }
            const path = await vscode.window.showInputBox({ placeHolder: '\u76f8\u5bf9\u8def\u5f84\uff08\u4f8b\u5982 src/app.py\uff09', value: defaultPath });
            if (!path) {
                vscode.window.showWarningMessage('\u8def\u5f84\u4e3a\u7a7a');
                return;
            }
            let content = ed ? ed.document.getText(ed.selection) : '';
            if (!content) {
                content = await vscode.window.showInputBox({ placeHolder: '\u5199\u5165\u5185\u5bb9\uff08\u7559\u7a7a\u5219\u53d6\u6d88\uff09' }) || '';
            }
            if (!content) {
                vscode.window.showWarningMessage('\u5185\u5bb9\u4e3a\u7a7a');
                return;
            }
            ...
            const runChecks = !mode.includes('strict') ? true : true;
        ...
        for (let i = 1; i <= n; i++) {
            ...
            if (!c) {
                c = await vscode.window.showInputBox({ placeHolder: `\u7b2c ${i} \u4e2a\u6587\u4ef6\u5185\u5bb9\uff08\u7559\u7a7a\u53d6\u6d88\u672c\u6b21\u591a\u6587\u4ef6\uff09` }) || '';
            }
            if (!c) {
                vscode.window.showWarningMessage('\u5185\u5bb9\u4e3a\u7a7a\uff0c\u5df2\u53d6\u6d88');
                break;
            }
            items.push({ path: p, content: c });
        }
        if (!items.length) {
            vscode.window.showWarningMessage('\u672a\u6536\u96c6\u5230\u6587\u4ef6');
            return;
        }
        ...
        try {
            ...
            try {
                await client.request('tools/call', { name: 'memory.append_turn', arguments: { role: 'assistant', content: `fs.apply_patch: dry-run multi, files=${items.length}`, meta: { source: 'vscode', action: 'fs.apply_patch', dryRun: true, files: items.length } } });
            }
            catch { }
        }
        catch (e) {
    ...
    const disposable = vscode.commands.registerCommand('mcpRulesAssistant.openPanel', async () => {
      ...
      <script>
        ...
        const panel = vscode.window.createWebviewPanel('mcpRulesAssistant', 'RuleFlow: Rules & Memory', vscode.ViewColumn.Beside, {
            enableScripts: true,
        ...
        const __preMsgs = [];
        if (!panel.__earlyHandlerInstalled) {
            panel.__earlyHandlerInstalled = true;
            panel.webview.onDidReceiveMessage(async (msg) => {
                // If full handler already set up, do nothing here
                if (panel.__fullHandlerReady)
                    return;
                try {
                    __preMsgs.push(msg);
                }
      ...
      <html>
        ...
        panel.onDidDispose(() => { __panelDisposed = true; try {
            __testPanelHandler = null;
        }
        catch { } });
        __panelReady = new Promise((res) => { __panelReadyResolve = res; });
        /* c8 ignore start */
        const render = (csp, nonceVal, md, toolsListHtml, sugg = '') => `
      <html>
      <head>
      ...
      <body class="simple" style="font-family: -apple-system,Segoe UI,Arial;">
        ...
        try {
            ...
            const list = (tools.tools || []).map((t) => `<li><code>${t.name}</code> \u2014 ${t.description}</li>`).join('');
            const csp = panel.webview.cspSource;
            ...
            try {
                ...
                if (ws) {
                    ...
                    try {
                        ...
                        if (obj && typeof obj.lang === 'string' && obj.lang)
                            lang = obj.lang;
                    }
            ...
            try {
                ...
                panel.webview.postMessage({ t: 'license', license: (diag && diag.license) || {} });
                // \u7f3a\u5c11\u5de5\u5177\u6216 venv \u65f6\uff0c\u63d0\u793a\u4e00\u952e\u51c6\u5907\u73af\u5883
                const toolsMap = (diag && diag.tools) || {};
                const missing = [];
                ['pytest', 'pre-commit', 'ruff', 'mypy', 'bandit'].forEach(k => { if (!toolsMap[k])
                    missing.push(k); });
        ...
        const __panelDispatch = async (msg) => {
            // mark in-flight for tests to await idle
            try {
                __panelInFlight = new Promise((res) => { __panelInFlightResolve = res; });
            }
            catch { }
            try {
                __testWebviewHandler = async (m) => { await handleOpenMessage(m); };
                if (msg.t === 'retryConnect') {
                    ...
                    try {
                        ...
                        client.fakeMode = true; // best-effort
                        await vscode.commands.executeCommand('mcpRulesAssistant.openPanel');
                        return;
                    }
                    catch (e) {
                    ...
                    try {
                        panel.webview.postMessage({ t: 'info', text: 'Webview \u5df2\u8fde\u63a5\uff08handshake_ok\uff09' });
                    }
                    catch { }
                }
                    ...
                    try {
                        ...
                        try {
                            let old = '';
                            try {
                                const b = await vscode.workspace.fs.readFile(p);
                                old = Buffer.from(b).toString('utf8');
                            }
                            catch { }
                            await vscode.workspace.fs.writeFile(p, enc.encode(old + line));
                        }
                        catch {
                            await vscode.workspace.fs.writeFile(p, enc.encode(line));
                        }
                    }
                    catch { }
                }
                    ...
                    try {
                        ...
                        vscode.window.showInformationMessage('\u672a\u627e\u5230 .mcp/dashboard/server.log');
                    }
                }
                    ...
                    try {
                        ...
                        const errs = Array.isArray(msg.errors) ? msg.errors.slice(-100) : [];
                        const diag = { time: new Date().toISOString(), workspace: ws, strict: String(process.env.MCP_STRICT_ISOLATION || ''), projectRootEnv: String(process.env.MCP_PROJECT_ROOT || '') };
                        try {
                            const ext = vscode.extensions.getExtension('ruleflow.mcp-rules-assistant');
                            diag.version = (ext && ext.packageJSON.version) || '';
                        }
                        catch { }
                        try {
                            diag.env = await client.request('tools/call', { name: 'env.diagnose', arguments: {} });
                        }
                        catch { }
                        try {
                            diag.audit = await client.request('tools/call', { name: 'security.audit_report', arguments: {} });
                        }
                        catch { }
                        try {
                            ...
                            diag.events_tail = lines.slice(-120);
                        }
                        catch { }
                    ...
                    try {
                        ...
                        try {
                            await vscode.workspace.fs.createDirectory(dash);
                        }
                        catch { }
                    ...
                    try {
                        ...
                        try {
                            panel.webview.postMessage({ t: 'setLang', value: next });
                        }
                        catch { }
                    }
                    catch { }
                    ...
                    (0, child_process_1.execFile)(pyBin, ['-m', 'mcp_rules_assistant.cli', 'status-update', '--json'], { cwd, env }, (err, stdout, stderr) => {
                        if (err) {
                            vscode.window.showErrorMessage('\u72b6\u6001\u5237\u65b0\u5931\u8d25\uff1a' + String(err));
                            return;
                        }
                ...
                if (msg.t === 'loadRules') {
                    ...
                    const compiledUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/compiled'))?.uri;
                    const jsonUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/compiled.json'))?.uri;
                    if (!compiledUri) {
                    ...
                    try {
                        ...
                        const jsonUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/compiled.json'))?.uri;
                        if (!jsonUri) {
                            vscode.window.showWarningMessage('\u672a\u627e\u5230\u7f16\u8bd1\u89c4\u5219\uff0c\u8bf7\u5148\u6444\u53d6\u89c4\u5219');
                            return;
                        }
                        const compiled = await client.request('resources/read', { uri: jsonUri });
                        const data = JSON.parse(compiled.text || '{}');
                        ...
                        const lines = [];
                        if (typeof minMod === 'number')
                            lines.push(`coverage.min_module = ${(minMod * 100).toFixed(0)}%`);
                        if (typeof minCore === 'number')
                            lines.push(`coverage.min_core = ${(minCore * 100).toFixed(0)}%`);
                        if (wantHadolint)
                            lines.push('ci.hadolint = true');
                        if (wantSemgrep)
                            lines.push('ci.semgrep_config = auto');
                        ...
                        if (confirm === '\u5e94\u7528') {
                            ...
                            try {
                                panel.webview.postMessage({ t: 'info', text: summary });
                            }
                            catch { }
                        }
                    }
                    ...
                    let weakCount = 0, groupsCount = 0, nearCount = 0;
                    let minModule = undefined;
                    try {
                        const rep = await client.request('tools/call', { name: 'coverage.report', arguments: {} });
                        if (rep && rep.ok) {
                            ...
                            if (typeof rep.min_module === 'number')
                                minModule = rep.min_module;
                        }
                    }
                    catch { }
                    const resList = await client.request('resources/list', {});
                    const covUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/summary'))?.uri;
                    const groupsUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/groups'))?.uri;
                    const treeUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/tree'))?.uri;
                    const nearUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/near'))?.uri;
                    if (!covUri) {
                    ...
                    if (treeUri) {
                        try {
                            ...
                            const tdata = JSON.parse(tres.text || '{}');
                            if (tdata.ok) {
                    ...
                    if (nearUri) {
                        try {
                            ...
                            const ndata = JSON.parse(nres.text || '{}');
                            if (ndata.ok) {
                    ...
                    try {
                        const parts = [`weak=${weakCount}`, `near=${nearCount}`, `groups=${groupsCount}`];
                        if (typeof minModule === 'number')
                            parts.push(`min_module=${(minModule * 100).toFixed(0)}%`);
                    ...
                    const tres = await client.request('resources/read', { uri: treeUri });
                    try {
                        const data = JSON.parse(tres.text || '{}');
                        if (!data.ok) {
                    ...
                    const resList = await client.request('resources/list', {});
                    const compiledUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/compiled'))?.uri;
                    const suggUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/suggestions'))?.uri;
                    const jsonUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/compiled.json'))?.uri;
                    if (compiledUri) {
                        const res = await client.request('resources/read', { uri: compiledUri });
                        panel.webview.postMessage({ t: 'rules', md: res.text || '' });
                    ...
                    try {
                        ...
                        if (a === 'openpanel' || a === 'panel') {
                            await vscode.commands.executeCommand('mcpRulesAssistant.openPanel');
                            return;
                        }
                        if (a === 'quick' || a === 'quickactions') {
                            await vscode.commands.executeCommand('mcpRulesAssistant.quickActions');
                            return;
                        }
                        if (a === 'coverage' || a === 'loadcoverage') {
                            await vscode.commands.executeCommand('mcpRulesAssistant.loadCoverage');
                            return;
                        }
                        if (a === 'status' || a === 'statusupdate') {
                            await vscode.commands.executeCommand('mcpRulesAssistant.statusUpdate');
                            return;
                        }
                        if (a === 'ingest' || a === 'ingestquick') {
                            await vscode.commands.executeCommand('mcpRulesAssistant.ingestQuick');
                            return;
                        }
                        if (a === 'openplan') {
                        ...
                        if (a === 'doctor' || a === 'backenddoctor') {
                            await vscode.commands.executeCommand('mcpRulesAssistant.backendDoctor');
                            return;
                        }
                        if (a === 'env.prepare' || a === 'prepareenv' || a === 'prepareenvinstall') {
                            await vscode.commands.executeCommand('mcpRulesAssistant.envPrepareInstall');
                            return;
                        }
                        if (a === 'ci.generate') {
                            await client.request('tools/call', { name: 'ci.generate', arguments: {} });
                        ...
                        if (a === 'installhooks' || a === 'git.install_hooks') {
                            await client.request('tools/call', { name: 'git.install_hooks', arguments: {} });
                            vscode.window.showInformationMessage('钩子安装完成');
                            return;
                        }
                    ...
                    const resList = await client.request('resources/list', {});
                    const memUri = (resList.resources || []).find((r) => String(r.uri || '').startsWith('memory://'))?.uri;
                    if (!memUri) {
                        panel.webview.postMessage({ t: 'memory', text: '\u65e0\u8bb0\u5fc6\u8d44\u6e90' });
                        return;
                    }
                    ...
                    const resList = await client.request('resources/list', {});
                    const planUri = (resList.resources || []).find((r) => String(r.uri || '').startsWith('progress://'))?.uri;
                    if (!planUri) {
                        panel.webview.postMessage({ t: 'plan', text: '\u65e0\u8ba1\u5212\u8d44\u6e90' });
                        return;
                    }
                    ...
                    if (act && act.startsWith('\u6807\u8bb0\u8fdb\u884c\u4e2d')) {
                        ...
                        if (cur)
                            await client.request('tools/call', { name: 'plan.set', arguments: { status: 'in_progress', current: cur } });
                    ...
                    try {
                        await client.request('tools/call', { name: 'memory.append_turn', arguments: { role: 'assistant', content: 'CI generated', meta: { source: 'vscode', action: 'ci.generate', path: out && out.path } } });
                    }
                    catch { }
                }
                    ...
                    try {
                        await client.request('tools/call', { name: 'memory.append_turn', arguments: { role: 'assistant', content: 'CI validated', meta: { source: 'vscode', action: 'ci.validate', ok: res && res.ok } } });
                    }
                    catch { }
                }
                else if (msg.t === 'ciPreviewInline') {
                    const resList = await client.request('resources/list', {});
                    const ciUri = (resList.resources || []).find((r) => String(r.uri || '').startsWith('ci://'))?.uri;
                    if (!ciUri) {
                        panel.webview.postMessage({ t: 'ciPreviewContent', text: '\uff08\u672a\u751f\u6210 CI\uff09' });
                        return;
                    }
                    ...
                    const ciUri = (resList.resources || []).find((r) => String(r.uri || '').startsWith('ci://'))?.uri;
                    if (!ciUri) {
                        vscode.window.showWarningMessage('\u672a\u627e\u5230 CI \u8d44\u6e90');
                        return;
                    }
                    const res = await client.request('resources/read', { uri: ciUri });
                    ...
                    try {
                        ...
                        try {
                            ...
                            await client.request('tools/call', { name: 'compliance.commitment', arguments: { write: true } });
                        }
                    ...
                    try {
                        ...
                        try {
                            ...
                            const lines = info.map((it) => {
                                if (it && typeof it === 'object') {
                                    return `${it.time || ''}  ${it.text || ''}`;
                                }
                                return String(it);
                            });
                            ...
                            panel.webview.postMessage({ t: 'info', text: '\u72b6\u6001\u6458\u8981\u89e3\u6790\u5931\u8d25' });
                        }
                    }
                    catch {
                        panel.webview.postMessage({ t: 'info', text: '\u672a\u627e\u5230 status.json' });
                    }
                }
                    ...
                    try {
                        ...
                        const msgInfo = (out && out.ok) ? ('\u5df2\u521b\u5efa\u5e76\u5b89\u88c5\uff1a' + String(out.venv || '')) : '\u6267\u884c\u5931\u8d25';
                        ...
                        try {
                            ...
                            await new Promise((resolve) => {
                                try {
                                    const p = (0, child_process_1.spawn)(creator, ['-m', 'venv', venvDir], { cwd: ws });
                                    p.on('close', () => resolve());
                                    p.on('error', () => resolve());
                                }
                                catch {
                            ...
                            const run = (args) => new Promise((resolve) => {
                                try {
                                    const p = (0, child_process_1.spawn)(vpy, args, { cwd: ws });
                                    p.on('close', () => resolve());
                                    p.on('error', () => resolve());
                                }
                                catch {
                    ...
                    try {
                        ...
                        try {
                            ...
                            let obj = {};
                            try {
                                obj = JSON.parse(fs.readFileSync(up, 'utf8') || '{}');
                            }
                            catch { }
                            obj.projectRoot = __lockedRoot;
                            fs.writeFileSync(up, JSON.stringify(obj, null, 2));
                        }
                        catch { }
                    ...
                    try {
                        ...
                        try {
                            await client.request('tools/call', { name: 'memory.append_turn', arguments: { role: 'assistant', content: 'Onboard applied', meta: { source: 'vscode', action: 'rules.onboard' } } });
                        }
                        catch { }
                    }
                    catch (e) {
                ...
                try {
                    if (__panelInFlightResolve) {
                        __panelInFlightResolve();
                    }
                }
                catch { }
        ...
        panel.webview.onDidReceiveMessage(async (msg) => {
            ...
            try {
                ...
                if (ws) {
                    ...
                    try {
                        let old = '';
                        try {
                            const b = await vscode.workspace.fs.readFile(uriE);
                            old = Buffer.from(b).toString('utf8');
                        }
                        catch { }
            ...
            if (msg && msg.t === 'ready') {
                try {
                    if (__panelReadyResolve) {
                        __panelReadyResolve();
                        __panelReadyResolve = null;
                    }
                }
                catch { }
            }
            ...
            if (msg && msg.t === 'nl') {
                try {
                    ...
                    const mapped = (res && res.parsed && res.parsed.tool) || '';
                    // \u7b80\u6613\u8c03\u5ea6\uff1a\u6839\u636e\u6620\u5c04\u8c03\u7528\u5e38\u7528\u5de5\u5177
                    ...
                    const runCoverage = async () => {
                        const resList = await client.request('resources/list', {});
                        const summaryUri = (resList.resources || []).find((r) => String(r.uri || '').endsWith('/summary'))?.uri;
                        if (!summaryUri) {
                            panel.webview.postMessage({ t: 'info', text: '\u672a\u627e\u5230\u8986\u76d6\u7387\u8d44\u6e90\uff0c\u8bf7\u5148\u751f\u6210 coverage.xml\u3002' });
                            return;
                        }
                    ...
                    if (mapped === 'rules.ingest')
                        await runIngest();
                    else if (mapped === 'coverage.near')
                        await runNear();
                    else if (mapped === 'resources.read') {
                        if (/\u8986\u76d6\u7387|coverage/.test(lower))
                            await runCoverage();
                        else
                            await runLoadRules();
                    }
                    else if (mapped === 'memory.toggle_auto')
                        await runToggleMemory();
                    else if (mapped === 'memory.snapshot')
                        await runMemorySnapshot();
                    else if (mapped === 'ci.generate')
                        await runCiGen();
                    else if (mapped === 'ci.validate')
                        await runCiValidate();
                    else if (mapped === 'ci.autofix')
                        await runCiAutofix();
                    else if (mapped === 'git.install_hooks')
                        await runInstallHooks();
                    else if (mapped === 'rules.validate') {
                        await client.request('tools/call', { name: 'rules.validate', arguments: {} });
                        await runLoadRules();
                    }
                    else if (mapped === 'rules.enforce')
                        await runEnforce();
                    else if (mapped === 'env.prepare')
                        await runPrepareEnv();
                    ...
                    const h = context.workspaceState.get('ruleflow.nl.history') || [];
            ...
            if (msg && msg.t === 'covNearPrompt') {
                try {
                    ...
                    const items = (res && res.near) ? res.near : [];
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_waitReady2', async (topic) => {
        try {
            const key = String(topic || 'any');
            let p = __ready2Promises.get(key);
            if (!p) {
                p = new Promise((res) => { __ready2Resolvers.set(key, res); });
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_simulateWebviewMessage', async (msg) => {
        if (__testWebviewHandler) {
            await __testWebviewHandler(msg);
        }
        return true;
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_getFakeCalls', async () => {
        try {
            return client._testGetFakeCalls ? client._testGetFakeCalls() : [];
        }
        catch {
            return [];
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_clearFakeCalls', async () => {
        try {
            if (client._testClearFakeCalls)
                client._testClearFakeCalls();
            return true;
        }
        catch {
            return false;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_chatSetEnabled', async (on) => {
        try {
            await context.workspaceState.update('ruleflow.chat.appendEnabled', !!on);
            return true;
        }
        catch {
            return false;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.chatAppendSummary', async (text) => {
        try {
            client.start(context);
        }
        catch { }
        try {
            const enabled = !!context.workspaceState.get('ruleflow.chat.appendEnabled');
            if (!enabled) {
                vscode.window.showInformationMessage('Chat \u8ffd\u52a0\u6458\u8981\u672a\u542f\u7528');
                return true;
            }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_dispatchQuick', async (which) => {
        try {
            switch (which) {
                case 'openWeakCsv': {
                    const ws = getWorkspaceRoot();
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_dispatchPanel', async (which) => {
        try {
            const ws = getWorkspaceRoot() || process.cwd();
            const dash = vscode.Uri.file(ws + '/.mcp/dashboard');
            try {
            ...
            switch (which) {
                ...
                case 'ciOpenExist': {
                    ...
                    try {
                        await vscode.workspace.fs.stat(yml);
                    }
                    catch {
                ...
                case 'ciOpenMissing': {
                    const yml = vscode.Uri.file(ws + '/.github/workflows/ci.yml');
                    try {
                        await vscode.workspace.fs.delete(yml, { recursive: false });
                    }
                    catch { }
                ...
                case 'covNearPrompt': {
                    ...
                    try {
                        await vscode.workspace.fs.stat(near);
                    }
                    catch {
                        await vscode.workspace.fs.writeFile(near, Buffer.from('file,coverage,threshold\n', 'utf-8'));
                    }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_sendPanelMessage', async (msg) => {
        if (__testPanelHandler) {
            await __testPanelHandler(msg);
            return true;
        }
        return false;
    }));
    // test-only: NL \u5386\u53f2 add/clear\uff08\u4e0d\u4f9d\u8d56\u540e\u7aef\uff09
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant._test_nlHistory', async (op, text) => {
        if (op === 'add') {
            const h = context.workspaceState.get('ruleflow.nl.history') || [];
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.nlCommand', async () => {
        try {
            ...
            if (!text)
                return;
            const res = await client.request('tools/call', { name: 'nl.command', arguments: { text } });
            const tool = (res && res.parsed && res.parsed.tool) || 'nl.command';
            const lower = (text || '').toLowerCase();
            ...
            if (tool === 'coverage.report' || /\u52a0\u8f7d\u8986\u76d6\u7387|load coverage/.test(lower)) {
                ...
                const stMap = { '\u8fdb\u884c\u4e2d': 'in_progress', '\u5b8c\u6210': 'done' };
        ...
        try {
                    ...
                    await new Promise((resolve) => {
                        try {
                            const creator = (process.env.MCP_PYTHON_BIN && process.env.MCP_PYTHON_BIN.trim())
                                ? process.env.MCP_PYTHON_BIN.trim()
                                : (process.platform === 'win32' ? 'python' : 'python3');
                            const p = (0, child_process_1.spawn)(creator, ['-m', 'venv', path.join(ws, '.mcp', 'venv')], { cwd: ws });
                            p.on('close', () => resolve());
                            p.on('error', () => resolve());
                        }
                        catch {
            ...
            const run = (args) => new Promise((resolve) => {
                try {
                    ...
                    const p = (0, child_process_1.spawn)(py, args, { cwd: ws, env });
                    let out = '', err = '';
                    ...
                    resolve({ code: 1, out: '', err: 'spawn exception' });
                }
            });
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcpRulesAssistant.installRemote', async () => {
        try {
            await context.workspaceState.update('ruleflow.autoInstallRemoteTried', false);
        }
        catch { }
        try {
...
async function runShell(cmd, args, cwd) {
    return new Promise((resolve, reject) => {
        const p = (0, child_process_1.spawn)(cmd, args, { cwd, shell: process.platform === 'win32' });
        let stderr = '';
...
async function autoInstallToRemote(context) {
    try {
        ...
        try {
            await vscode.workspace.fs.createDirectory(remoteIdeDir);
        }
        catch { }
...
async function oneClickSetup(context) {
    try {
        client.start(context);
        const ws = getWorkspaceRoot() || process.cwd();
        ...
        try {
            const defaultPaths = [];
            try {
                if (require('fs').existsSync(require('path').join(ws, 'README.md')))
                    defaultPaths.push('README.md');
            }
            catch { }
            try {
                if (require('fs').existsSync(require('path').join(ws, 'docs')))
                    defaultPaths.push('docs/');
            }
            catch { }
...
async function newbieGuide(context) {
    try {
        client.start(context);
        ...
        if (prep && prep.startsWith('创建')) {
            try {
                await client.request('tools/call', { name: 'env.prepare', arguments: { create: true, install: true } });
                vscode.window.showInformationMessage('已准备环境。');
            }
            catch (e) {
                vscode.window.showWarningMessage('准备环境失败：' + String(e));
            }
        }
        // Step 3: Ingest rules
        ...
        if (ingest && ingest.startsWith('快速')) {
            try {
                ...
                vscode.window.showWarningMessage('摄取失败：' + String(e));
            }
        }
        ...
        try {
            ...
            vscode.window.showWarningMessage('CI/Hooks 步骤遇到问题：' + String(e));
        }
        // Finish
...
async function askCommand(context) {
    try {
        ...
        const lower = text.toLowerCase();
        if (/摄取|ingest/.test(lower)) {
            await client.request('tools/call', { name: 'rules.ingest', arguments: { paths: ['README.md', 'docs/'] } });
            vscode.window.showInformationMessage('规则摄取完成。');
            return;
        }
        if (/覆盖率|coverage|load coverage/.test(lower)) {
            await vscode.commands.executeCommand('mcpRulesAssistant.loadCoverage');
            return;
        }
        if (/ci/.test(lower) && /生成|generate/.test(lower)) {
            await client.request('tools/call', { name: 'ci.generate', arguments: {} });
            vscode.window.showInformationMessage('CI 已生成。');
            return;
        }
        if (/钩子|hooks|install hooks/.test(lower)) {
            await client.request('tools/call', { name: 'git.install_hooks', arguments: {} });
            vscode.window.showInformationMessage('Git hooks 已安装。');
            return;
        }
        if (/准备|prepare|env/.test(lower)) {
            await client.request('tools/call', { name: 'env.prepare', arguments: { create: true, install: true } });
            vscode.window.showInformationMessage('环境已准备。');
            return;
        }
