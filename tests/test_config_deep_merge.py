...
def chdir(path: Path):
    class _Ctx:
        def __enter__(self):
            self._old = Path.cwd(); os.chdir(path); return path
...
def test_load_config_deep_merge_preserves_nested_defaults(tmp_path: Path) -> None:
    with chdir(tmp_path):
        ...
        assert abs(float(cfg['performance']['on_push']['coverage']['min_core']) - 0.95) < 1e-6
        # overridden min_module applied
        assert abs(float(cfg['performance']['on_push']['coverage']['min_module']) - 0.93) < 1e-6
