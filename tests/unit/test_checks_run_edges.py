...
def test__compute_tests_signature_stat_error(monkeypatch, tmp_path: Path) -> None:
    ...
    def bad_stat(self, *, follow_symlinks: bool = True):  # type: ignore[override]
        if self == target:
            raise OSError("nope")
        # Preserve signature compatibility with pathlib.Path.stat in Python 3.8+
...
def test_run_quick_tests_config_yaml_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        checks,
        "_run",
        lambda *a, **k: {"ok": True, "code": 0, "stdout": "", "stderr": ""},
    )
