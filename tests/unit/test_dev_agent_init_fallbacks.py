...
def test_run_else_calls_instance_method_when_module_not_callable(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(dev_agent, "_run_impacted_or_full", lambda *a, **k: {"ok": True, "mode": "full"})
    monkeypatch.setattr(
        dev_agent,
        "compute_status",
        lambda *a, **k: {"plan": {"status": "in_progress", "current": "x", "next": "y"},
