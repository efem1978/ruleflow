...
from mcp_rules_assistant.process import run_cmd, DEFAULT_TIMEOUT
...
def test_run_cmd_logging_and_nocapture(tmp_path: Path, monkeypatch) -> None:
    ...
    p2 = run_cmd(["bash", "-lc", "true"], cwd=tmp_path, capture_stdout=False, env=None, timeout=DEFAULT_TIMEOUT)
...
def test_run_cmd_retry_on_timeout(monkeypatch, tmp_path: Path) -> None:
    ...
    p = run_cmd(["bash", "-lc", "echo after"], cwd=tmp_path, retries=1, retry_on_timeout_only=True, capture_stdout=True, log=True)
