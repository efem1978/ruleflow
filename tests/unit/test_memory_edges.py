...
from mcp_rules_assistant.memory import MemoryManager, DEFAULT_MEMORY_FILE
...
def test_add_link_with_nonlist_links(tmp_path: Path) -> None:
    ...
    p.write_text(json.dumps({"turns": [], "summary": "", "links": {}}, ensure_ascii=False), encoding="utf-8")
...
def test_compress_continue_on_nondict(tmp_path: Path) -> None:
    ...
    data = {
        'turns': [ 'bad_entry', {'role':'assistant','content':'X'*300,'meta':{}}, {'role':'user','content':'Y'*300,'meta':{}} ],
        'summary': '', 'links': []
    }
