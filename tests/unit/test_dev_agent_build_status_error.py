...
def test_build_current_status_error_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(agent, "compute_status", lambda *a, **k: (_ for _ in ()).throw(RuntimeError("boom")))
    tests = {"ok": False}
    bypass = {"active": False}
    checks = {"lint": "ok", "type": "skipped", "tdd": "skipped"}
    st = agent._build_current_status(tests, bypass, checks, interval=1, cmd_error_count=0)
