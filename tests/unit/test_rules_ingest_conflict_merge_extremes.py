...
def test_numeric_close_values_keep_stricter_without_conflict(tmp_path: Path) -> None:
    raw = {
        "items": [
            {"key": "coverage.min_core", "value": 0.960, "text": "a", "source": {"file": "a.md", "line": 1}},
            {"key": "coverage.min_core", "value": 0.961, "text": "b", "source": {"file": "b.md", "line": 2}},
        ]
    }
...
def test_boolean_conflict_detected_and_stricter_kept(tmp_path: Path) -> None:
    raw = {
        "items": [
            {"key": "test.no_skip_xfail", "value": False, "text": "a", "source": {"file": "a.md", "line": 1}},
            {"key": "test.no_skip_xfail", "value": True, "text": "b", "source": {"file": "b.md", "line": 2}},
        ]
    }
    (tmp_path / '.mcp').mkdir(parents=True, exist_ok=True)
    (tmp_path / '.mcp/rules_raw.json').write_text(json.dumps(raw), encoding='utf-8')
    ...
    assert any(c.get('key') == 'test.no_skip_xfail' for c in (out.get('conflicts') or []))
