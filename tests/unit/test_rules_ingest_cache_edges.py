...
def test_ingest_text_cache_miss_and_exceptions(monkeypatch, tmp_path: Path) -> None:
    ...
    a = root / 'a.md'; a.write_text('- coverage 90%\n', encoding='utf-8')
    b = root / 'b.md'; b.write_text('- coverage 95%\n', encoding='utf-8')
    c = root / 'c.md'; c.write_text('- coverage 96%\n', encoding='utf-8')
    # create a JSON file to hit YAML/JSON branch
    d = root / 'd.json'; d.write_text('{"x": 1}', encoding='utf-8')
    ...
    (root / '.mcp/rules_ingest_cache.json').write_text(json.dumps(cache), encoding='utf-8')
