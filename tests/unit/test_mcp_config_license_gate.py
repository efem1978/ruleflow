...
def test_config_update_license_gate_nested_ci(tmp_path: Path) -> None:
    srv = JsonRpcServer()
    ...
    try:
        srv._call_tool("config.update", {"data": {"ci": {"semgrep_config": "auto"}}})
        assert False, "expected license gate error"
    except Exception as e:
        assert "license" in str(e).lower()


def test_config_update_license_gate_flat_key(tmp_path: Path) -> None:
    srv = JsonRpcServer()
    srv.project_root = tmp_path
    (tmp_path / ".mcp").mkdir(parents=True, exist_ok=True)
