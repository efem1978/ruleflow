...
def test_run_tests_with_coverage_success(
    ...
    monkeypatch.setattr(dev_agent, "run_cmd", lambda *a, **k: P())
...
def test_run_impacted_or_full_quick_and_full(
    ...
    monkeypatch.setattr(
        ...
        lambda *_: {"ok": True, "code": 0, "stdout": "", "stderr": ""},
    )
...
def test_main_autocommit_and_tag(
    ...
    fake_cov = {
        "plan": {"status": "in_progress", "current": "Step1", "next": "N"},
        "coverage": {
            "weak": [],
            "groups": [],
            "near": [],
    ...
    def fake_run(cmd, *, cwd=None, capture_stdout=True, env=None, check=False):
    ...
    monkeypatch.setattr(dev_agent_module, "run_cmd", fake_run)
