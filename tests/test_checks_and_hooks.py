...
from mcp_rules_assistant.hooks import generate_pre_commit_config, install_git_hooks
...
def test_render_github_ci_yaml_respects_compiled_rules(tmp_path: Path) -> None:
    ...
    assert "Check Dockerfile existence" in yml  # container.required triggers Dockerfile check
...
def test_plan_gate_script_requires_step_token(tmp_path: Path) -> None:
    ...
    p = subprocess.run([sys.executable, str(tmp_path / ".mcp/plan_gate.py"), "commit-msg", str(msg)], cwd=tmp_path)
    assert p.returncode != 0
    # With token should pass
    msg.write_text("feat: impl [step:实现A]", encoding="utf-8")
    p2 = subprocess.run([sys.executable, str(tmp_path / ".mcp/plan_gate.py"), "commit-msg", str(msg)], cwd=tmp_path)
...
def test_fs_apply_patch_strict_rejects_skip_marker(tmp_path: Path) -> None:
    ...
    try:
        srv._call_tool(
            "fs.apply_patch",
            {
                "files": [{"path": "tests/test_x.py", "content": "import pytest\n@pytest.mark.skip\ndef test_a(): pass\n"}],
                "runChecks": True,
                "strict": True,
            },
        )
...
def test_quick_tests_ordering_uses_failure_history_and_recency(tmp_path: Path) -> None:
    ...
    data = {
        ...
        "events": [
            {"nodeid": "tests/test_slow.py::test_a", "file": t1, "ts": str(now - 10*24*3600)},
            {"nodeid": "tests/test_flaky.py::test_b", "file": t2, "ts": str(now - 3600)},
        ],
    }
...
def test_dockerfile_baseline_gate(tmp_path: Path) -> None:
    ...
    compiled = tmp_path / ".mcp/rules_compiled.json"
    compiled.parent.mkdir(parents=True, exist_ok=True)
    compiled.write_text('{"policy": {"container.policy.baseline": true}}', encoding="utf-8")
    install_git_hooks(tmp_path)
...
def test_no_skip_xfail_grep_command(tmp_path: Path) -> None:
    ...
    subprocess.run(['git', 'add', '-A'], cwd=tmp_path, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    # The grep used in pre-commit should detect the marker
    r = subprocess.run(['git', 'grep', '-nE', r"pytest\.mark\.(skip|xfail)", '--', '.'], cwd=tmp_path)
    assert r.returncode == 0  # found

    # Now replace content to remove markers and ensure not found
    bad.write_text("def test_ok(): assert 1\n", encoding='utf-8')
    subprocess.run(['git', 'add', '-A'], cwd=tmp_path, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    r2 = subprocess.run(['git', 'grep', '-nE', r"pytest\.mark\.(skip|xfail)", '--', '.'], cwd=tmp_path)
