...
def test_suggestions_have_severity_and_cli_summarizes(tmp_path: Path) -> None:
    ...
    (m / 'rules_compiled.json').write_text((tmp_path / '.mcp' / 'rules_compiled.json').read_text(encoding='utf-8') if (tmp_path / '.mcp' / 'rules_compiled.json').exists() else '', encoding='utf-8')
    ...
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        (Path('.mcp') / 'rules_compiled.json').write_text((tmp_path / '.mcp' / 'rules_compiled.json').read_text(encoding='utf-8'), encoding='utf-8')
        r = runner.invoke(app, ['rules-explain', '--json'])
        assert r.exit_code == 0
        out = r.stdout or ''
        assert '"suggestions_severity"' in out and '"must"' in out and '"warn"' in out and '"info"' in out
