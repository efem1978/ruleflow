...
def test_resource_coverage_report_json(tmp_path: Path) -> None:
    ...
    rep_uri = next(r['uri'] for r in rlist['result']['resources'] if str(r['uri']).endswith('/report'))
    rep = srv.handle({"jsonrpc":"2.0","id":1,"method":"resources/read","params":{"uri": rep_uri}})
...
def test_resources_memory_and_ping(tmp_path: Path) -> None:
    srv = JsonRpcServer(); srv.project_root = tmp_path
    rlist = srv.handle({"jsonrpc":"2.0","id":1,"method":"resources/list","params":{}})
    mem_uri = next(r['uri'] for r in rlist['result']['resources'] if str(r['uri']).endswith('/rollup'))
    mem = srv.handle({"jsonrpc":"2.0","id":1,"method":"resources/read","params":{"uri": mem_uri}})
