...
def test_env_diagnose_reads_maxima(tmp_path: Path) -> None:
    ...
    (d / 'rules_compiled.json').write_text('{"meta": {"maxima": {"coverage.max_module": 0.95}}}', encoding='utf-8')
...
def test_rules_enforce_with_invalid_yaml(tmp_path: Path) -> None:
    ...
    assert out.get('ok') is True and any(str(s).startswith('coverage.min_module=') for s in (out.get('enforced') or []))
