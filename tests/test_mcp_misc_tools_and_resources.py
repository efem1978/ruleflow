...
def test_mcp_project_and_memory_and_rules_init(tmp_path: Path) -> None:
    ...
    # rules.init
    rinit = srv._call_tool("rules.init", {"scenario": "enterprise", "complexity": "large", "devMode": "tdd"})
...
def test_mcp_rules_resources_after_ingest(tmp_path: Path) -> None:
    ...
    (tmp_path / '.mcp/rules_suggestions.md').write_text('# Suggestions\n', encoding='utf-8')
...
def test_mcp_tools_env_and_config_and_ci_and_enforce(tmp_path: Path) -> None:
    ...
    assert nl.get('ok') is True and isinstance(nl.get('parsed', {}).get('tool'), (str, type(None)))
    # config.get + update
    cfg = srv._call_tool('config.get', {"section": None})
    assert cfg.get('ok') is True and isinstance(cfg.get('config'), dict)
    upd = srv._call_tool('config.update', {"data": {"hadolint": True, "semgrep_config": "auto"}})
    ...
    (tmp_path / '.mcp/rules_compiled.json').write_text('{"policy": {"coverage.min_module": 0.91}}', encoding='utf-8')
    en = srv._call_tool('rules.enforce', {})
    assert en.get('ok') is True and 'coverage.min_module' in '\n'.join(en.get('enforced', []) or [])
...
def test_mcp_tool_error_paths(tmp_path: Path) -> None:
    ...
    (tmp_path / '.mcp').mkdir(parents=True, exist_ok=True)
    (tmp_path / '.mcp/rules_compiled.json').write_text('{invalid json', encoding='utf-8')
    try:
