...
from .progress import ensure_plan, read_plan, write_plan, update_plan_fields
import yaml
import platform, shutil, json as _json
...
def init() -> None:
    ...
    rprint("[green]✔ Created[/] .mcp/assistant.yaml with performance defaults (Fast mode)")
...
def ingest_rules(
    ...
    if res.get("compiled", {}).get("conflicts"):
        rprint("[yellow]存在规则冲突，请查看 .mcp/rules_compiled.md 的 Conflicts 段落[/]")
    else:
        rprint("[green]无显著冲突[/]")
...
def rules_validate() -> None:
    ...
    if not res.get("ok"):
        rprint(f"[yellow]{res.get('message', '尚未摄取规则，请先运行 ingest-rules')}[/]")
        raise typer.Exit(1)
...
def rules_explain(
    ...
    with_suggestions: str = typer.Option("none", "--with-suggestions", help="建议输出：none/short"),
) -> None:
    ...
    if json_out:
        import json as _json
        payload: dict = {
            "coverage": {"min_module": pol.get('coverage.min_module'), "min_core": pol.get('coverage.min_core')},
            "flags": {k: bool(pol.get(k)) for k in ['test.no_skip_xfail','test.warnings_as_errors','test.mutation_required','security.secrets_scan','security.sast_strict','container.required','container.policy.baseline'] if k in pol},
            "conflicts": len(conflicts),
            "suggestions": len(sugg),
            "maxima": {k: maxima.get(k) for k in sorted(maxima.keys())} if maxima else {},
            "suggestions_severity": {
                "must": sum(1 for s in sugg if isinstance(s, dict) and s.get('severity') == 'must'),
                "warn": sum(1 for s in sugg if isinstance(s, dict) and s.get('severity') == 'warn'),
                "info": sum(1 for s in sugg if isinstance(s, dict) and s.get('severity') == 'info'),
            },
        }
        if with_suggestions.lower() == "short":
            payload["suggestions_keys"] = [str(x.get("key")) for x in sugg if isinstance(x, dict) and x.get("key")]
        print(_json.dumps(payload, ensure_ascii=False))
        return
    lines = []
    mm = pol.get('coverage.min_module'); mc = pol.get('coverage.min_core')
    lines.append(f"coverage.min_module={mm if mm is not None else 'N/A'}; coverage.min_core={mc if mc is not None else 'N/A'}")
    for k in [
...
def coverage() -> None:
    ...
    for it in weak:
        delta = float(it.get('delta', float(it.get('threshold', 0)) - float(it.get('coverage', 0))))
        rprint(f" - {it['coverage']*100:.1f}% < {int((it.get('threshold', 0))*100)}% (Δ {delta*100:.1f}%) — {it['file']}")
...
def coverage_groups() -> None:
    ...
    for g in res.get("groups", []):
        rprint(f" - {g['prefix']}: {g['coverage']*100:.1f}% < {int((g.get('threshold',0))*100)}% — 弱项 {g['weak_count']}/{g['files_count']}")
...
def coverage_tree() -> None:
    ...
    def walk(node: dict, prefix: str = "") -> None:
        ...
        for f in files[:5]:
            rprint(f"{prefix}  - {f.get('coverage',0)*100:.1f}% < {int((f.get('threshold',0))*100)}% — {f.get('file')}")
...
def coverage_near(
    within: Optional[float] = typer.Option(None, help="距阈值百分比（例如 3 = 3%），留空使用配置 coverage.near.within"),
    top: Optional[int] = typer.Option(None, help="显示前 N 个，留空使用配置 coverage.near.top"),
    ...
    min_module = float((perf.get("on_push", {}) or {}).get("coverage", {}).get("min_module", 0.9))
    policy = (cfg.get("coverage", {}) or {}).get("policy", None) if isinstance(cfg.get("coverage", {}), dict) else None
    near_cfg = (cfg.get("coverage", {}) or {}).get("near", {}) if isinstance(cfg.get("coverage", {}), dict) else {}
    within_pct = float(near_cfg.get("within", 0.03) * 100.0) if within is None else float(within)
    top_n = int(near_cfg.get("top", 20)) if top is None else int(top)
    within_pct = max(1.0, min(10.0, within_pct))
    res = cov_near(policy=policy, min_module=min_module, within=within_pct / 100.0, top=top_n)
    if not res.get("ok"):
        rprint(f"[yellow]{res.get('message', 'coverage.xml 不存在')}[/]")
        raise typer.Exit(1)
    near = res.get("near", [])
    ...
    if fmt == "csv":
        ...
        if output:
            with open(output, 'w', newline='', encoding='utf-8') as f:
                ...
                for it in near:
                    w.writerow([it.get('file',''), it.get('coverage',0.0), it.get('threshold',0.0), it.get('delta_up',0.0)])
        else:
            import sys
    ...
    for it in near:
        ...
        rprint(f" - {it['coverage']*100:.1f}% ≥ {int((it.get('threshold', 0))*100)}% (距阈值 {delta_up*100:.1f}%) — {it['file']}")
...
def coverage_report(
    json_out: bool = typer.Option(True, '--json/--text', help='以 JSON 输出（默认）或文本'),
    within: Optional[float] = typer.Option(None, help='近阈值窗口（百分比），留空用配置'),
    ...
    res_near = cov_near(policy=policy, min_module=min_module, within=within_pct/100.0, top=top_n)
...
def diagnose(
    ...
    if compiled_exists:
        try:
            data = _json.loads(Path('.mcp/rules_compiled.json').read_text(encoding='utf-8'))
            meta = data.get('meta', {}) or {}
...
def rules_suggestions(
    ...
    if fmt == 'csv':
        ...
        for s in sugg:
            if isinstance(s, dict):
                rows.append([
                    s.get('key',''), s.get('action',''), s.get('severity',''), s.get('value',''), s.get('note','')
                ])
        if output:
            with open(output, 'w', newline='', encoding='utf-8') as f:
                w = csv.writer(f)
    ...
    for s in sugg:
        if isinstance(s, dict):
            key = s.get('key',''); act = s.get('action',''); sev = s.get('severity',''); val = s.get('value', None)
            if val is not None:
...
def plan_update(text: str = typer.Argument(..., help="计划内容 Markdown（可粘贴）")) -> None:
...
def ci_set(
    ...
    semgrep_config: Optional[str] = typer.Option(None, help="semgrep 规则集（如 auto/p/ci 等）"),
    ...
    vscode_required: Optional[bool] = typer.Option(None, help="是否强制在 CI 中运行 VS Code 扩展测试（不再按文件存在性判断）"),
) -> None:
...
def ci_autofix() -> None:
    ...
    rprint({"path": res.get("path"), "changed": res.get("changed"), "backup": res.get("backup")})
...
def prepare_env(
    ...
    dry_run: bool = typer.Option(False, help="仅输出计划，不执行（等效于 --no-create --install False）"),
    packages: Optional[str] = typer.Option(None, help="自定义安装包列表，逗号分隔（在 --install 时生效）"),
) -> None:
...
def coverage_near_set(
    within: float = typer.Option(3.0, help="距阈值百分比（例如 3 = 3%），范围建议 1–10"),
