...
from .rules import Scenario, Complexity, DevMode, choose_thresholds, explain_thresholds
...
from .config import load_config, DEFAULT_PROJECT_CONFIG_PATH, ensure_project_config
...
class JsonRpcServer:
    ...
    def handle(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if method == "initialize":
                ...
                tools = [asdict(t) if hasattr(t, "__dict__") else {"name": t.name, "description": t.description} for t in registry.list()]
                result = {"tools": tools}
                ...
                result = {"resources": [
                    {"uri": f"memory://{self._project_id()}/rollup", "name": "Last 20 turns & summary"},
                    {"uri": f"rules://project/{self._project_id()}/compiled", "name": "Compiled project rules"},
                    {"uri": f"rules://project/{self._project_id()}/compiled.json", "name": "Compiled rules (JSON)"},
                    {"uri": f"rules://project/{self._project_id()}/suggestions", "name": "Rule conflicts & suggestions"},
                    {"uri": f"rules://project/{self._project_id()}/maxima", "name": "Coverage upper-bounds (maxima)"},
                    {"uri": f"coverage://project/{self._project_id()}/summary", "name": "Coverage summary"},
                    {"uri": f"coverage://project/{self._project_id()}/groups", "name": "Coverage groups"},
                    {"uri": f"coverage://project/{self._project_id()}/tree", "name": "Coverage tree (weak)"},
                    {"uri": f"coverage://project/{self._project_id()}/near", "name": "Coverage near threshold"},
                    {"uri": f"coverage://project/{self._project_id()}/report", "name": "Coverage report (weak/groups/near)"},
                    {"uri": f"progress://{self._project_id()}/plan", "name": "Project plan"},
                    {"uri": f"config://project/{self._project_id()}/assistant.yaml", "name": "Project config (YAML)"},
                    {"uri": f"ci://project/{self._project_id()}/workflow", "name": "CI workflow (YAML)"},
                ]}
            elif method == "resources/read":
                uri = params.get("uri", "")
                if uri.startswith("memory://"):
                    snap = self.mm.snapshot()
                    result = {"mimeType": "application/json", "text": json.dumps(snap, ensure_ascii=False)}
                    ...
                    if uri.endswith("/compiled"):
                        ...
                        result = {"mimeType": "application/json", "text": _json.dumps({"maxima": maxima}, ensure_ascii=False)}
                        return {"jsonrpc": "2.0", "id": req_id, "result": result}
                    else:
                        raise ValueError("unknown rules resource")
                    if not path.exists():
                        raise ValueError("rules resource not found; run rules.ingest first")
                    result = {"mimeType": mime, "text": path.read_text(encoding="utf-8")}
                elif uri.startswith("coverage://"):
                    # 动态读取当前项目配置，避免初始化时的 cwd 影响（测试/多项目场景）
                    cfg_now = load_config(self.project_root)
                    perf = cfg_now.get("performance", {}) if isinstance(cfg_now.get("performance", {}), dict) else {}
                    min_module = float((perf.get("on_push", {}) or {}).get("coverage", {}).get("min_module", 0.9))
                    # 未来可扩展 per-module policy；当前读取 config 中的 coverage.policy 可选字段
                    policy = (cfg_now.get("coverage", {}) or {}).get("policy", None) if isinstance(cfg_now.get("coverage", {}), dict) else None
                    if uri.endswith("/groups"):
                        summary = covsum.summarize_groups(project_root=self.project_root, policy=policy, min_module=min_module)
                        result = {"mimeType": "application/json", "text": json.dumps(summary, ensure_ascii=False)}
                    elif uri.endswith("/tree"):
                        summary = covsum.summarize_tree(project_root=self.project_root, policy=policy, min_module=min_module)
                        result = {"mimeType": "application/json", "text": json.dumps(summary, ensure_ascii=False)}
                    elif uri.endswith("/near"):
                        # 默认窗口与 Top 可由配置覆盖（coverage.near）
                        near_cfg = (self.cfg.get("coverage", {}) or {}).get("near", {}) if isinstance(self.cfg.get("coverage", {}), dict) else {}
                        within = float((near_cfg or {}).get("within", 0.03))
                        top = int((near_cfg or {}).get("top", 50))
                        summary = covsum.summarize_near(project_root=self.project_root, policy=policy, min_module=min_module, within=within, top=top)
                        result = {"mimeType": "application/json", "text": json.dumps(summary, ensure_ascii=False)}
                    elif uri.endswith("/report"):
                        near_cfg = (self.cfg.get("coverage", {}) or {}).get("near", {}) if isinstance(self.cfg.get("coverage", {}), dict) else {}
                        within = float((near_cfg or {}).get("within", 0.03))
                        top = int((near_cfg or {}).get("top", 50))
                        res_sum = covsum.summarize(project_root=self.project_root, policy=policy, min_module=min_module)
                        res_grp = covsum.summarize_groups(project_root=self.project_root, policy=policy, min_module=min_module)
                        res_near = covsum.summarize_near(project_root=self.project_root, policy=policy, min_module=min_module, within=within, top=top)
                        payload = {
                            "ok": bool(res_sum.get("ok") and res_grp.get("ok", True) and res_near.get("ok", True)),
                            "weak": res_sum.get("weak", []),
                            "groups": res_grp.get("groups", []),
                            "near": res_near.get("near", []),
                            "min_module": min_module,
                        }
                        result = {"mimeType": "application/json", "text": json.dumps(payload, ensure_ascii=False)}
                    else:
                        summary = covsum.summarize(project_root=self.project_root, policy=policy, min_module=min_module)
                        result = {"mimeType": "application/json", "text": json.dumps(summary, ensure_ascii=False)}
    ...
    def _call_tool(self, name: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if name == "rules.init":
            ...
            return {"scenario": s.value, "complexity": c.value, "devMode": dm.value, "thresholds": explain_thresholds(th)}
        ...
        if name == "env.prepare":
            ...
            packages = list(args.get("packages") or [
                "ruff", "black", "isort", "mypy", "bandit", "pytest", "pytest-cov", "pre-commit",
            ])
            ...
            try:
                if create:
                    ...
                    subprocess.run([py, "-m", "venv", str(venv_dir)], cwd=self.project_root, check=False)
                    created = True
                if install:
                    py_bin = (venv_dir / ("Scripts" if os.name == "nt" else "bin") / ("python.exe" if os.name == "nt" else "python"))
                    # 升级 pip 与安装工具链（尽量不中断）
                    subprocess.run([str(py_bin), "-m", "pip", "install", "--upgrade", "pip"], cwd=self.project_root, check=False)
                    if packages:
                        subprocess.run([str(py_bin), "-m", "pip", "install", *packages], cwd=self.project_root, check=False)
                    installed = True
            except Exception as e:
                return {"ok": False, "message": f"env prepare failed: {e}", "plan": plan, "venv": str(venv_dir)}
            return {"ok": True, "created": created, "installed": installed, "venv": str(venv_dir), "plan": plan}
        ...
        if name == "coverage.near":
            ...
            near_cfg = (cfg_now.get("coverage", {}) or {}).get("near", {}) if isinstance(cfg_now.get("coverage", {}), dict) else {}
            within = float(args.get("within", near_cfg.get("within", 0.03)))
            top = int(args.get("top", near_cfg.get("top", 50)))
            perf = cfg_now.get("performance", {}) if isinstance(cfg_now.get("performance", {}), dict) else {}
            min_module = float((perf.get("on_push", {}) or {}).get("coverage", {}).get("min_module", 0.9))
            policy = (cfg_now.get("coverage", {}) or {}).get("policy", None) if isinstance(cfg_now.get("coverage", {}), dict) else None
            data = covsum.summarize_near(project_root=self.project_root, policy=policy, min_module=min_module, within=within, top=top)
            return data
        ...
        if name == "fs.apply_patch":
            ...
            for f in files:
                ...
                if run_checks and strict:
                    ...
                    if "pytest.mark.skip" in content or "pytest.mark.xfail" in content:
                        raise ValueError("检测到 skip/xfail 标记，受控写入被拒绝（strict）。")
                p = Path(f["path"])
                if not dry_run:
            ...
            if run_checks and not dry_run:
                do_type = bool(self.cfg.get("performance", {}).get("on_commit", {}).get("typecheck_incremental", True))
                result_checks = checks_mod.run_checks(changed_paths, cwd=self.project_root, do_lint=True, do_type=do_type, do_quick_tests=True)
                if strict and not result_checks.get("ok", True):
                    raise ValueError("受控写入后的检查未通过（lint/type/tests）")
            out: Dict[str, Any] = {"ok": True, "written": 0 if dry_run else len(files), "checks": result_checks}
            if dry_run:
        ...
        if name == "plan.set":
            ...
            nxt = args.get("next")
            progress_mod.update_plan_fields(self.project_root, status=status, current=current, nxt=nxt)
            return {"ok": True}
        if name == "config.get":
        ...
        if name == "ci.validate":
            ...
            checks = {
                ...
                "has_mutation": ("mutmut run" in content) or ("Mutation testing" in content),
                "has_node_tests": ("VS Code extension tests" in content) and ((self.project_root / "extensions/vscode/package.json").exists()),
                "has_python_matrix": "matrix:\n        python-version" in content or "matrix:\n      python-version" in content,
                "has_pip_cache": "cache: 'pip'" in content or 'cache: "pip"' in content or "cache: pip" in content,
                "has_npm_cache": "cache: 'npm'" in content or 'cache: "npm"' in content or "cache: npm" in content,
                ...
                "has_combined_artifact": ("coverage.xml" in content and "pytest-junit.xml" in content and "near.txt" in content),
        ...
        if name == "rules.enforce":
            ...
            policy = compiled.get("policy", {}) if isinstance(compiled.get("policy", {}), dict) else {}
            ...
            on_push = perf.get("on_push", {}) if isinstance(perf.get("on_push", {}), dict) else {}
            cov = on_push.get("coverage", {}) if isinstance(on_push.get("coverage", {}), dict) else {}
            changed = False
            ...
            ci = data.get("ci", {}) if isinstance(data.get("ci", {}), dict) else {}
            ci_changed = False
            if bool(policy.get("container.required") or policy.get("container.policy.baseline")) and not bool(ci.get("hadolint", False)):
                ci["hadolint"] = True
                ci_changed = True
            if bool(policy.get("security.sast_strict")) and not ci.get("semgrep_config"):
            ...
            if bool(policy.get("security.secrets_scan")) or bool(policy.get("container.policy.baseline")):
                needs_hooks = True
            # 写回配置
            if changed:
                cfg_path.write_text(yaml.safe_dump(data, sort_keys=False, allow_unicode=True), encoding="utf-8")
            ...
            return {"ok": True, "updated": {"coverage": cov, "ci": data.get("ci", {})}, "changed": bool(changed), "enforced": enforced, "needs_hooks": needs_hooks, "needs_ci_regen": needs_ci_regen}
    ...
    def _tool_project_detect(self) -> Dict[str, Any]:
        ...
        if (root / "pyproject.toml").exists() or list(root.glob("**/*.py")):
            lang = "python"
        elif (root / "package.json").exists() or list(root.glob("**/*.ts")) or list(root.glob("**/*.js")):
            lang = "node"
